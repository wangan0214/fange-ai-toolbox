...
- **环境**：managed python 无 playwright → 用 **Node 版 playwright**（`/Users/fanshuai/.workbuddy/binaries/node/workspace/node_modules/playwright`）+ 系统 Chrome（`/Applications/Google Chrome.app/Contents/MacOS/Google Chrome`）。
...
- 单会话 Playwright 渲染（无头 Chrome 起一次逐页截图）
